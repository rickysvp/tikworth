...
export function GrowthPlanSection({ plan }: { plan: GrowthPlan }) {
  ...
  return (
    <div className="rounded-2xl border border-[#FF0050]/30 bg-[#FF0050]/5 p-6">
      <h3 className="text-sm font-semibold uppercase tracking-wider text-[#FF0050] mb-2">{dict.evaluation.growthPlan.title}</h3>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full border ${item.priority === 'high' ? 'border-red-900/50 text-red-400' : item.priority === 'medium' ? 'border-amber-900/50 text-amber-400' : 'border-green-900/50 text-green-400'}`}>
                    {item.priority === 'high' ? dict.evaluation.growthPlan.highPriority : item.priority === 'medium' ? dict.evaluation.growthPlan.mediumPriority : dict.evaluation.growthPlan.lowPriority}
                  </span>
                </div>
                <p className="text-sm text-neutral-300 mb-1">{item.action}</p>
                <p className="text-xs text-neutral-500">{dict.evaluation.growthPlan.expected} {item.expectedImpact}</p>
              </div>
            </div>
          </div>
        ))}
