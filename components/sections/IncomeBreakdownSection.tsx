  ...
  return (
    <div className="rounded-2xl border border-neutral-800 bg-[#0f0f0f] p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-sm font-semibold uppercase tracking-wider text-neutral-500">预估月收入</h3>
      ...
      <div className="mb-6 rounded-2xl border border-[#00F2EA]/20 bg-gradient-to-br from-[#00F2EA]/5 to-[#FF0050]/5 p-5">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-neutral-500 mb-1 uppercase tracking-wider">预估月收入范围</div>
            <div className="text-3xl font-bold">
              {hasIncome ? `${formatUsd(total.low)} - ${formatUsd(total.high)}` : '$0'}
            </div>
            {hasIncome && (
              <div className="text-sm text-neutral-400 mt-1">
                中位数 {formatUsd(total.mid)}
              </div>
            )}
          </div>
          <div className="hidden sm:flex items-center gap-2 rounded-full border border-neutral-700 px-4 py-2">
            ...
            <span className="text-sm text-neutral-400">
              地区倍率 {estimate.regionMultiplier}x
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-neutral-500">
            {source.confidence === 'high' ? '高可信' : source.confidence === 'medium' ? '中可信' : '低可信'}
          </span>
        </div>
        <div className="text-right">
          {hasIncome ? (
            ...
            <span className="text-sm text-neutral-600">暂不适用</span>
          )}
        </div>
      </div>
